...
import SAMUS.utils.metrics as metrics # added SAMUS
from hausdorff import hausdorff_distance
from SAMUS.utils.visualization import visual_segmentation, visual_segmentation_binary, visual_segmentation_sets, visual_segmentation_sets_with_pt
from einops import rearrange
from SAMUS.utils.generate_prompts import get_click_prompt
import time
import pandas as pd
...
def eval_mask_slice(valloader, model, criterion, opt, modelname):
    model.eval()
    ...
    for batch_idx, (datapack) in enumerate(valloader):
        ...
        if modelname == 'MSA' or modelname == 'SAM':
...
def eval_mask_slice2(valloader, model, criterion, opt, modelname):
    model.eval()
    val_losses, mean_dice = 0, 0
...
def eval_camus_patient(valloader, model, criterion, opt, modelname):
    model.eval()
    val_losses, mean_dice = 0, 0
    dices = np.zeros(opt.classes)
...
def eval_patient(valloader, model, criterion, opt, modelname):
    model.eval()
    val_losses, mean_dice = 0, 0
    dices = np.zeros(opt.classes)
...
def eval_slice(valloader, model, criterion, opt, modelname):
    model.eval()
    val_losses, mean_dice = 0, 0
    max_slice_number = opt.batch_size * (len(valloader) + 1)
    dices = np.zeros((max_slice_number, opt.classes))
...
def eval_camus_samed(valloader, model, criterion, opt, modelname):
    model.eval()
    val_losses, mean_dice = 0, 0
    classes = 4
...
def get_eval(valloader, model, criterion, opt, modelname):
    if modelname == "SAMed":
    ...
    if opt.eval_mode == "mask_slice":
        return eval_mask_slice2(valloader, model, criterion, opt, modelname)
    elif opt.eval_mode == "slice":
        return eval_slice(valloader, model, criterion, opt, modelname)
    elif opt.eval_mode == "camusmulti":
        return eval_camus_patient(valloader, model, criterion, opt, modelname)
    elif opt.eval_mode == "patient":
        return eval_patient(valloader, model, criterion, opt, modelname)
    elif opt.eval_mode == "camus_samed":
        return eval_camus_samed(valloader, model, criterion, opt, modelname)
    else:
